...
var buildMaps = function buildMaps(prefix, actionAndSagaMap, defaultTakeEffect, checkAndWarn, logBuilt) {
    ...
    (0, _keys2.default)(actionAndSagaMap).forEach(function (actionName) {
        ...
        if (logBuilt) {
            console.log("Saga actionCreator: " + actionName + "(" + actionArgumentNames.join(", ") + ")   " + "--->   type: '" + actionType + "'");
        }
    });
...
var getSagaArgNames = function getSagaArgNames(saga, actionType) {
    if (saga != null) {
        var reducerArgs = _functionArgNames2.default.getArgs(saga);

        if (reducerArgs.length > 0) {
            var firstArg = reducerArgs[0];
            if (firstArg.substr(0, 4) === "_ref") {
                var refArgs = _functionArgNames2.default.getRefs(saga, firstArg);
                if (refArgs == null) {
