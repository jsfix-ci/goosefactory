...
var getSagaArgNames = function getSagaArgNames(func, actionType) {
    if (func != null) {
        ...
        if (reducerArgs.length > 0) {
            ...
            if (firstArg.substr(0, 4) === "_ref") {
                var refArgs = _functionArgNames2.default.getRefs(func, firstArg);
                if (refArgs == null) {
                    console.warn("Possibly flawed action '" + actionType + "': the saga generator expects a deconstructed object ( e.g. {name1, name2, name3} ) as its " + "first argument, but this seems empty");
                }
                return refArgs || [];
            } else if (firstArg !== "action") {
                console.warn("Possibly flawed reducer for action " + actionType + ": the saga generator expected 'action' as the name of its first argument");
                return [];
            }
        }
    }
...
var GooseFactory = function () {
    function GooseFactory(actionPrefix, actionAndSagaMapOrMaps) {
        var _this = this;
